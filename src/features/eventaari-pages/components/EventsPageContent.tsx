...
function toEventItem(e: any): EventItem {
  const when = new Date(e.starts_at);
  const calendarDate = getLocalDateKey(when);
  return {
    id: String(e.id),
    title: e.name,
    category: e.categories?.name ?? "Community",
...
export default function EventsPageContent() {
  ...
  useEffect(() => {
    getEvents().then((rows: any[]) => setLiveEvents(rows.map(toEventItem)));
  }, []);
  ...
  useEffect(() => {
    ...
    setCalendarDate((current) =>
      synchronizeCalendarDate(current, [...liveEvents, ...customEvents]),
    );
      ...
      <main className="page-content events-page-content">
        <section className="event-search-panel">
          ...
          <input
            ...
            type="search"
            placeholder="Try Music, Food or a location"
            value={search}
            onChange={(event) => {
